#!/usr/bin/env sh
# SessionStart hook: injects the full i-have-work ruleset when the user has
# opted in by creating $CLAUDE_CONFIG_DIR/.i-have-work-always (default ~/.claude).
...
flag_path="$claude_dir/.i-have-work-always"
...
skill_path="$script_dir/../skills/i-have-work/SKILL.md"
[ -f "$skill_path" ] || exit 0
...
printf 'I-HAVE-WORK MODE ACTIVE (always-on). The ruleset below applies to every response. "stop work mode" turns it off for this session; delete %s to turn always-on off for good.\n\n%s\n' \
  "$flag_path" "$body"
